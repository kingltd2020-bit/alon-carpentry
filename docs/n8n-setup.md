...
| **ה-workflow** | ✅ **פורסם ופעיל ב-n8n** — נבנה מתוך [`alon-leads.workflow.json`](alon-leads.workflow.json) |
| **Production URL** | ✅ `https://kingltd2020.app.n8n.cloud/webhook/alon-lead` |
| `WEBHOOK_URL` ב-`index.html` | ✅ **מולא** |

### מה השתנה (‏2026-08-31)

עד כאן ה-workflow היה **קובץ ייבוא בלבד** — ה-MCP של n8n לא היה מחובר,
ולכן הוא לא נבנה בתוך n8n ולא הופעל. עכשיו ה-workflow קם, פורסם,
וה-Production URL חי ומודבק בדף.

המשמעות: שליחה מהדף יוצאת עכשיו ל-n8n באמת, ולא נופלת להודעת השגיאה.

### מה עדיין פתוח

| # | פריט | מצב |
|---|---|---|
| 1 | הכרעה על **הווב-הוק הפתוח** — שלושת הפתרונות בסעיף הבא | ⚠️ להכרעה |
| 2 | `Allowed Origins` — להחליף `*` בדומיין האמיתי אחרי הפריסה | ⚠️ לאמת |
| 3 | שבע הבדיקות שבסוף המסמך — מהדף החי, מהטלפון | ⚠️ להרצה |
...
```js
const WEBHOOK_URL = 'https://kingltd2020.app.n8n.cloud/webhook/alon-lead';
```

אם השורה הזו תתרוקן שוב, כל שליחה תיפול להודעת *"השליחה לא עברה —
כנראה הרשת. אפשר לנסות שוב."* השדות נשארים מלאים, כך שלחיצה חוזרת
לא דורשת להקליד מחדש.

הכתובת גלויה לכל מי שפותח את קוד המקור של הדף. זה לא באג — דף סטטי
לא יכול להסתיר כתובת שהדפדפן של הגולש צריך לפנות אליה. בגלל זה ההגנה
חייבת לשבת בצד n8n — ראה הסעיף על הווב-הוק הפתוח.

### הבדיקות האוטומטיות לא נוגעות ב-n8n

`tools/check.js` מחליף את `fetch` בכישלון מזויף לפני בדיקת מסלול
הכישלון, ו-`tools/check-success.js` מחליף אותו בתשובת 200 מזויפת.
**אף אחת מההרצות לא שולחת ליד אמיתי** לווב-הוק ולא מייצרת שורה
בגיליון. את הצד של n8n בודקים ידנית, לפי הטבלה למטה.
